...
class DcdReader:
  ...
  def __init__(self, fname):
    ...
    if self._read_fmt_val('i') <> 84 :
      raise Exception(""DCD format error 2"")
    size = self._read_fmt_val('i')
    if (size - 4) % 80 <> 0 :
      raise Exception(""DCD format error 3"")
